...
const columns = [
  ...
  },
  {
    ...
    render: (val) => <CTable.NumberTpl value={val} precision={0} prefix="¥" />,
  },
  {
    title: "订单数",
    dataIndex: "orderNum",
...
export default function CTableDemo() {
  const sort = (data, { sortParams }) => {
    const { dataIndex, sortBy } =
      sortParams?.allSortColumns?.find((item) => item.sortBy) || {};
    if (dataIndex === "id") {
    ...
    if (
      ["name", "createTime", "num", "orderNum", "creator"].includes(dataIndex)
    ) {
      return data.sort((a, b) =>
        sortBy === "ASC"
  ...
  return (
    <CTable
      ...
      ajaxData={(params) => {
        console.log("所有列排序配置:");
        console.table(
          params.sortParams?.allSortColumns?.map((item) => {
            return {
