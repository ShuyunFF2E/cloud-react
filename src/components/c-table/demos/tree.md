...
export default function CTableDemo() {
  ...
  const onExpand = (expanded, record) => {
    ...
    expandedRowKeys.splice(
      expandedRowKeys.findIndex((key) => key === record.key),
      1
    );
  ...
  const onExpandAll = () => {
    ...
    if (isExpandAll) {
      setExpandedRowKeys(
        Array.from(
          new Set([...currentData.map((item) => item.key), ...expandedRowKeys])
        )
      );
      return;
    }
    setExpandedRowKeys(
      expandedRowKeys.filter(
        (key) => !currentData.find((item) => item.key === key)
      )
    );
  };
  ...
  return (
    <div>
      <Button style={{ marginBottom: 20 }} onClick={onExpandAll}>
        {isExpandAll ? "展开全部" : "收起全部"}
      </Button>
      <CTable
