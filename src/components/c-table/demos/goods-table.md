...
function createGuid() {
  let guid = "";
  for (let i = 1; i <= 2; i += 1) {
    guid += Math.random().toString(32).substr(2);
  }
...
const data = new Array(100).fill(1).map((item, index) => ({
  ...
  skus:
    index === 0
      ? []
      : new Array(3).fill(1).map((item, index1) => {
          return {
...
class CTableDemo extends Component {
  ...
  constructor(props) {
    ...
    setTimeout(() => {
      ...
      this.setState({
        defaultShowColumns: this.columns
          .slice(0, 4)
    ...
    eles.forEach((ele, index) => {
      if (ele.classList.contains("custom-row-disabled")) {
        ...
        if (targetProductId) {
          ...
          const i = checkedGoodsData.findIndex(
            (goods) => String(goods.productId) === String(targetProductId)
          );
          if (i > -1) {
        ...
        if (this.state.currentData[index]) {
          ...
          const curSkusIds = this.state.currentData[index].skus.map(
            (sku) => sku.productId
          );
          curSkusIds.forEach((id) => {
            const i = checkedSkusData.findIndex(
              (sku) => String(sku.productId) === String(id)
            );
            if (i > -1) {
      ...
      render: (_, row) => {
        ...
        return (
          <div>
            <Checkbox
              defaultChecked={
                !!checkedSkusData.find(
                  (sku) => String(sku.productId) === String(row.productId)
                )
              }
              onChange={(checked) => {
                if (checked) {
                  ...
                  const index = checkedSkusData.findIndex(
                    (sku) => String(sku.productId) === String(row.productId)
                  );
                  if (index > -1) {
  ...
  onExpand = (expanded, row) => {
    if (expanded) {
      this.setState(
        {
          expandedRowKeys: Array.from(
            new Set([...this.state.expandedRowKeys, row.productId])
          ),
        },
        () => {
          this.updateBtnText();
      ...
      const index = expandedRowKeys.findIndex(
  ...
  onExpandAll = () => {
    ...
    if (this.state.isExpandAll) {
      const currentIds = Array.from(
        new Set([
          ...this.state.currentData.map((item) => item.productId),
    ...
    this.tableRef.current.setColumn([
  ...
  render() {
    return (
      <div
        className="cloud-table-goods"
        style={{ height: 400, overflow: "auto" }}
      >
        <Button style={{ marginBottom: 20 }} onClick={this.onExpandAll}>
          {this.state.isExpandAll ? "展开全部" : "收起全部"}
        </Button>
        <CTable
          supportConfigColumn
          defaultShowColumns={this.state.defaultShowColumns}
          disabledConfigColumns={this.columns
            .slice(0, 2)
            .map((item) => item.dataIndex)}
