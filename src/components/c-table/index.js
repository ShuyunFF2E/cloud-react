...
class CTable extends Component {
	...
	componentDidUpdate(prevProps) {
		if (this.props.ajaxData !== prevProps.ajaxData || this.props.columnData !== prevProps.columnData) {
			this.init();
		}
	}
	...
	isInCurrentPage = targetKey => {
		...
		traverseTree(this.state.data, node => {
			if (String(this.getKey(node)) === targetKey) {
	...
	getLeafNodesMap = tree => {
		...
		traverseTree(tree, node => {
			LeafNodesMap[this.getKey(node)] = getLeafNodes(node);
		});
		return LeafNodesMap;
	};
	...
	setColumnData = () => {
		...
		if (supportCheckbox) {
			const currentLeafNodes = Object.keys(leafNodesMap).reduce((nodeList, key) => {
			...
			const checkboxColumn = {
				title: <Checkbox checked={isCheckedAll} indeterminate={isIndeterminateAll} onChange={checked => onAllCheckedChange(checked)} />,
				dataIndex: 'checkbox',
				key: 'checkbox',
				width: 40,
				fixed: columnData[0].fixed,
				render: (value, row) => {
					const isChecked = !!isEveryChecked(leafNodesMap[this.getKey(row)]);
					const isIndeterminate = !isChecked && isSomeChecked(leafNodesMap[this.getKey(row)]);
					return <Checkbox checked={isChecked} indeterminate={isIndeterminate} onChange={checked => onNodeCheckedChange(checked, row)} />;
				}
			};
			this.setState({
	...
	setHeaderHeight = () => {
		if (this.ref.current && this.ref.current.querySelector) {
			const headerEle = this.ref.current.querySelector(`.${tablePrefixCls}-thead`);
			this.setState({
	...
	updateSelectedNodes = (callback = () => {}) => {
		...
		traverseTree([...this.state.data, ...this.props.checkedData], node => {
			const key = this.getKey(node);
			const leafNodes = this.leafNodesMap[key];
			Object.assign(this.selectedNodeMap, { [key]: leafNodes.every(lNode => lNode.checked) ? node : null });
		});

		// 根据 selectedNodeMap 更新 selectedNodeList（selectedNodeList 给到外部使用）
		const selectedNodeList = Object.keys(this.selectedNodeMap).reduce((list, key) => {
	...
	onNodeCheckedChange = (checked, row) => {
		// 更新叶子节点 leafNodesMap 的选中状态
		this.leafNodesMap[this.getKey(row)].forEach(node => {
			Object.assign(node, { checked });
		});
		this.setColumnData();
		this.updateSelectedNodes(() => {
	...
	refreshGrid = async (pageNum, pageSize) => {
		this.setState({ isLoading: true }, async () => {
			...
			const { totals, data } = await getDataSource(this.props.ajaxData, _pageOpts);
			...
			<div>
				<div
					...
					<RcTable
						...
						// )}
					/>
					<Loading className={`${tablePrefixCls}-loading-layer`} loading={isLoading} />
				</div>
				{supportPage && (
					<div className={classnames(`${tablePrefixCls}-footer`)}>
						<div className={classnames(`${tablePrefixCls}-footer-statistics`)}>
							{supportCheckbox && <span className={classnames(`${tablePrefixCls}-footer-total`)}>已选 {selectedNodeList.length} 条</span>}
							<span className={classnames(`${tablePrefixCls}-footer-refresh`)} onClick={onRefresh}>
								<Icon type="refresh" />
								刷新
							</span>
						</div>
						<Pagination {...pageOpts} current={pageNum} pageSize={pageSize} total={totals} onChange={onPageChange} />
					</div>
				)}
				{footerTpl()}
			</div>
