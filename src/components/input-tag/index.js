...
export default class InputTag extends Component {
  ...
  handleKeyUp = (event) => {
    ...
    if (keyCode === 'Enter' && value.trim().length) {
      const values = this.sign
        ? value.split(this.sign).filter((item) => item)
        : value;
  ...
  computedInputWidth = () => {
    ...
    const eles = this.containerRef.current.querySelectorAll(
      `.${selector}-item`,
    );
    ...
    this.inputRef.current.style.cssText = `width: ${parseInt(
      inputWidth,
      10,
    )}px`;
  };
