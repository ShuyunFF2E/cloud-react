...
const OptionsSearch = ({
  ...
  <div className={`${selector}-search`}>
    <Input
      ...
      suffix={(
        <Icon
          className={`${selector}-search-input-icon ${
            searchValue ? 'hidden' : ''
          }`}
          type="search"
        />
    ...
    const search = (node) => node.filter((item) => {
      ...
      return (
        (!item.children || !item.children.length)
          && item.label.indexOf(String(searchText)) !== -1
      );
    });
        ...
        <div key={node.value} className={`${selector}-option-list`}>
          ...
          >
            ...
            {node.children && node.children.length ? (
              <Icon type="right" className={iconClassNames} />
            ) : null}
          </div>
          {node.children && node.children.length
            ? node.children.map((cNode) => child({ node: cNode, parentNode: node }))
            : null}
        </div>
      );
    };
