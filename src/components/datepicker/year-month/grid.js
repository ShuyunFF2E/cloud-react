    ...
    return (
      <div className="grid">
        <table className="grid-table year-grid-table">
          <tbody>
            {Array.from({ length: 4 }).map((o, index) => {
              ...
              const cls1 = this.getClassName(
                tempMonth,
                index1 + 1,
                currentMonth,
              );
              const cls2 = this.getClassName(
                tempMonth,
                index2 + 1,
                currentMonth,
              );
              const cls3 = this.getClassName(
                tempMonth,
                index3 + 1,
                currentMonth,
              );
              return (
                <tr key={index.toString()}>
                  <td className={classNames('grid-item', cls1)}>
                    <span onClick={() => this.onUpdate(index1, cls1)}>
                      {monthArr[index1]}
                    </span>
                  </td>
                  <td className={classNames('grid-item', cls2)}>
                    <span onClick={() => this.onUpdate(index2, cls2)}>
                      {monthArr[index2]}
                    </span>
                  </td>
                  <td className={classNames('grid-item', cls3)}>
                    <span onClick={() => this.onUpdate(index3, cls3)}>
                      {monthArr[index3]}
                    </span>
                  </td>
                </tr>
        ...
        <div className={`${selectorClass}-popup-btns`}>
          <Button
            size="small"
            disabled={this.getMonthDisabled()}
            onClick={() => this.onSave(currentMonth)}
          >
            当月
          </Button>
          <Button
