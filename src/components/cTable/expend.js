...
export default function getExpandableConfig({ supportExpend, supportTree, expandable, onExpand, expandedRowRender }) {
	if (!supportExpend) {
		return {};
	}
	return {
		...
		expandIcon: expendProps => {
			const hideExpendIcon = supportTree && (!expendProps.record.children || !expendProps.record.children.length);
			return <CustomExpandIcon expendProps={expendProps} hideIcon={hideExpendIcon} />;
		},
		onExpand: args => {
			if (onExpand) {
				onExpand(args);
