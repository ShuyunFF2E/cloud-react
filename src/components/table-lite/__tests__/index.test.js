...
describe('TableLite', () => {
  ...
  beforeAll(() => {
    columnData = [
      ...
      },
      {
        key: 'components',
        text: '参与组件',
        template: (components) => components ? components.join(', ') : '--',
      },
      {
        key: 'other',
        text: '其它贡献',
  ...
  it('正常render', () => {
    ...
    expect(wrapper.find('.cloud-table-lite-header')).toHaveLength(1);
    expect(wrapper.find('.cloud-table-lite-main')).toHaveLength(1);
    expect(wrapper.find('table')).toHaveLength(2);
    expect(wrapper.find('thead')).toHaveLength(2);
    expect(wrapper.find('tbody')).toHaveLength(1);
    expect(wrapper.find('tbody tr')).toHaveLength(2);
    expect(wrapper.find('tbody tr.cloud-table-lite-empty')).toHaveLength(0);
  });
  ...
  it('数据为空', () => {
    ...
    expect(wrapper.find('tbody tr.cloud-table-lite-empty')).toHaveLength(1);
  });
  ...
  it('树结构', () => {
    ...
    // 通过事件展开一个父节点
    let action = wrapper.find('i.cloud-icon.icon-right');
