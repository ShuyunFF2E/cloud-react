...
export default class SubMenu extends PureComponent {
  ...
  renderSubTitle() {
    ...
    return (
      <div className="group-title" style={style} onClick={this.handleToggle}>
        {title}
        <Icon type={iconType} className="indicator" />
      </div>
    );
  }
  ...
  render() {
    ...
    return (
      <li>
        {this.renderSubTitle()}
        <ul className={subGroupClass}>{this.props.children}</ul>
      </li>
    );
  }
}
