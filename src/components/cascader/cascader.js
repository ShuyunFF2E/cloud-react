...
const arrayTreeFilter = (data, filterFn, options) => {
	...
	let level = 0;
	do {
		const foundItem = children.filter(item => {
...
class CascaderMenu extends Component {
	...
	getActiveOptions(activeValue) {
		return arrayTreeFilter(this.props.options || [], (o, level) => o[this.getFieldName('value')] === activeValue[level], {
			childrenKeyName: this.getFieldName('children')
		});
	}
	...
	render() {
		const { prefixCls, transitionName, popupClassName, options = [], disabled, builtinPlacements, popupPlacement, children, ...restProps } = this.props;
