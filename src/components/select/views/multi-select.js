...
const ConfirmBtn = ({ onOk, onCancel, okBtnText, cancelBtnText, confirmTemplate }) => {
				...
				<div className={`${selector}-multiple-options`}>
					<div className={`${selector}-option-list`}>
						{hasSelectAll && !!views.length && (
							<Checkbox checked={checkAll} indeterminate={indeterminate} onChange={handleCheckAll} className={`${selector}-option`}>
								全选
							</Checkbox>
						)}
						{views}
