...
function TitleComponents(props) {
	return (
		<a href={'https://www.lovejavascript.com/#!zone/blog/content.html?id=' + props.row.id} target={'_black'}>
			{props.row.title}
		</a>
	);
}
	...
	return [
		{
			...
			text: '缩略图',
			template: (pic, row) => {
				return <img style={{ width: '88px', height: '58px', margin: '0 auto' }} src={'https://www.lovejavascript.com' + pic} title={row.name} />;
			}
		},
		{
			key: 'title',
