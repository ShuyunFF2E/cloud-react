...
export default class NewVision extends Component {
	...
	getPages = () => {
		...
		if (totalPage <= this.state.pagesLength){
			for (let i = 1; i <= totalPage; i += 1) {
				pages.push(<li onClick={() => this.goPage(i)} className={current === i ? "active" : ""} key={i}>{i}</li>)
			}
		} else {
			...
			if (this.state.current <= offset) {
				for (let i = 1; i <= offset + 1; i+=1) {
					pages.push(<li key={i} className={current === i ? "active" : ""} onClick={() => this.goPage(i)}>{i}</li>);
				}
				...
				pages.push(<li key={totalPage} className={current === totalPage ? "active" : ""} onClick={() => this.goPage(totalPage)}>{totalPage}</li>);
			} else if (this.state.current > totalPage - offset) {
			// 左边有。。。
				pages.push(<li key="1" className={current === 1 ? " active" : ""} onClick={() => this.goPage(1)}>1</li>);
				...
				for (let i = offset; i >= 1; i-=1) {
					pages.push(<li key={totalPage - i} className={current === totalPage - i ? " active" : ""} onClick={() => this.goPage(totalPage - i)}>{totalPage - i}</li>);
				}
				pages.push(<li key={totalPage} className={current === totalPage ? " active" : ""} onClick={() => this.goPage(totalPage)}>{totalPage}</li>);
			} else {
			// 两边都有。。。
				...
				for (let i = offset / 2; i >= 0; i-=1) {
					pages.push(<li key={current - i} className={current === current - i ? " active" : ""} onClick={() => this.goPage(current - i)}>{current - i}</li>);
				}
				for (let j = 1; j <= offset / 2; j+=1) {
					pages.push(<li key={current + j} className={current === current + j ? " active" : ""} onClick={() => this.goPage(current + j)}>{current + j}</li>);
				}
				pages.push(<li key="nextMore" className="ellips" ><span className="dot"></span><Icon type="doubleRight" className="moreIcon" onClick={() => this.nextMore()}></Icon></li>);
				pages.push(<li key={totalPage} className={current === totalPage ? "active" : ""} onClick={() => this.goPage(totalPage)}>{totalPage}</li>);
			}
	...
	render() {
		return (
			<div className="now">
				<ul>
					<li onClick={this.prevPage} className={this.state.current === 1 ? 'nomore' : ''}><Icon type="left" className="pg-icon"></Icon></li>
					{this.getPages()}
					<li onClick={this.nextPage} className={this.state.current === this.state.totalPage ? 'nomore' : ''}><Icon type="right" className="pg-icon"></Icon></li>
				</ul>
				{this.getJumper()}
