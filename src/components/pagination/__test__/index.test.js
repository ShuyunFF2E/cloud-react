...
describe('Pagination', () => {
  ...
  it('pageNum should change when input valid value', () => {
    ...
    inputWrapper.simulate('keydown', { nativeEvent: { keyCode: 13 } });
    expect(inputWrapper.getDOMNode().value).toBe('2');

    inputWrapper.getDOMNode().value = '3';
    inputWrapper.simulate('keydown', { nativeEvent: { keyCode: 13 } });
    expect(inputWrapper.getDOMNode().value).toBe('3');
  ...
  it('should trigger onChange when change pageSize', () => {
    ...
    wrapper.find('.change-size .cloud-select').simulate('change');
    expect(wrapper.state().current).toBe(1);
    wrapper.unmount();
  });
  ...
  it('support nextMore', () => {
    ...
    wrapper.find('.ellips .cloud-icon.icon-double-right').simulate('click');
    expect(wrapper.state().current).toBe(6);
  ...
  it('support preMore', () => {
    ...
    wrapper.find('.ellips .cloud-icon.icon-double-left').simulate('click');
    expect(wrapper.state().current).toBe(45);
    wrapper.unmount();

    const wrapper1 = mount(<Test total={500} current={5} />);
    wrapper1.find('.ellips .cloud-icon.icon-double-left').simulate('click');
