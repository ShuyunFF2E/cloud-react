...
describe('Tabs', () => {
  function initGetters(activeKey = 'tab1') {
    ...
    jest.spyOn(Tabs.prototype, 'tabsOffsetLeft', 'get').mockImplementation(() => configs.tabsOffsetLeft[activeKey]);

    jest.spyOn(Tabs.prototype, 'activeTabsOffsetWidth', 'get').mockImplementation(() => 192);
    jest.spyOn(Tabs.prototype, 'activeTabsOffsetLeft', 'get').mockImplementation(() => configs.activeTabsOffsetLeft[activeKey]);
  }
  ...
  it('renders correctly', () => {
    class TabsComponent extends Component {
      render() {
        return (
          <Tabs>
            <Tabs.Panel key="tab" tab="tab">tab</Tabs.Panel>
          </Tabs>
        );
      }
    }
  ...
  it('should support close tab', () => {
    ...
    wrapper.find('.cloud-icon.icon-close').at(1).simulate('click');
    expect(
      wrapper
  ...
  it('renders correctly when activeEle is null', () => {
    jest.spyOn(Tabs.prototype, 'activeEle', 'get').mockImplementationOnce(() => null);
    mount(<Test />);
  ...
  it('renders correctly when child is invalid', () => {
    mount(
      <Tabs activeKey="tab1">
        tab2
        <Tabs.Panel key="tab1" tab="tab1">tab1</Tabs.Panel>
      </Tabs>,
    );
    expect(true).toBeTruthy();
  });
  ...
  it('onClose is undefined', () => {
    ...
    wrapper.find('.cloud-icon.icon-close').at(0).simulate('click');
