...
export default class Tabs extends PureComponent {
	...
	static getDerivedStateFromProps(nextProps, state) {
		...
		if (prevProps.activeKey !== nextProps.activeKey || (prevProps.activeKey === nextProps.activeKey && prevChildCount !== nextChildCount)) {
			return {
	...
	get itemsNextLeft() {
		const { tabsOffsetLeft, tabsOffsetWidth, tabsScrollWidth } = this;
		return -tabsOffsetLeft + tabsOffsetWidth > tabsScrollWidth - this.step ? this.itemsMaxLeft : `${tabsOffsetLeft - this.step}px`;
	}

	get itemsPrevLeft() {
		const { tabsOffsetLeft } = this;
		return tabsOffsetLeft + this.step > 0 ? 0 : `${tabsOffsetLeft + this.step}px`;
	}
